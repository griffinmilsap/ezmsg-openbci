...
class OpenBCISerial(ez.Unit):
    ...
    async def pub_data(self) -> AsyncGenerator:
        ...
        while True:
            ...
            out_msg = TSMessage(data=cur_signal, fs=self.SETTINGS.sampling_rate)
            # Backward compatibility with EEGMessage
...
class OpenBCIImpedance(ez.Unit):
    ...
    async def on_data(self, message: TSMessage) -> AsyncGenerator:

        if not self.SETTINGS.enable:

            n_time = message.n_time
            window = np.hamming(n_time)

            # TODO: Assumes time_dim is 0 and ch_dim is 1
            fft = np.fft.fft((message.data.T * window)) / n_time  # ch x freq
            freqs = np.fft.fftfreq(n_time, d=1.0 / message.fs)
            imp_idx = np.argmin(np.abs(freqs - (message.fs / 4.0)))  # our impedance tone idx
